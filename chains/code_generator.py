...
Use ThreadPoolExecutor(max_workers=8) to parallelize getInfo() across the divided time blocks because getInfo() calls are synchronous and if they are run parallely it would give faster execution.
Filter and select:
...
Use ThreadPoolExecutor(max_workers=8) to parallelize getInfo() across the divided time blocks
